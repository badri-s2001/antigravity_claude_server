 ...
 * @returns True if the part is a thinking block
 */
export function isThinkingPart(part: ThinkingPart | AnthropicContentBlock | GooglePart): boolean {
  const p = part as ThinkingPart;
  return p.type === "thinking" || p.type === "redacted_thinking" || p.thinking !== undefined || p.thought === true;
}

/**
 * Check if a thinking part has a valid signature (>= MIN_SIGNATURE_LENGTH chars)
 */
export function hasValidSignature(part: ThinkingPart): boolean {
 ...
 */
export function sanitizeThinkingPart(part: ThinkingPart): ThinkingPart {
  // Gemini-style thought blocks: { thought: true, text, thoughtSignature }
  if (part.thought === true) {
    const sanitized: ThinkingPart = { thought: true };
  ...
  if (part.type === "thinking" || part.thinking !== undefined) {
    const sanitized: ThinkingPart = { type: "thinking" };
 ...
 */
export function sanitizeAnthropicThinkingBlock(block: ThinkingPart): ThinkingPart {
  ...
  if (block.type === "redacted_thinking") {
    const sanitized: ThinkingPart = { type: "redacted_thinking" };
 ...
 */
function filterContentArray(contentArray: (ThinkingPart | GooglePart)[]): (ThinkingPart | GooglePart)[] {
  const filtered: (ThinkingPart | GooglePart)[] = [];
  ...
  for (const item of contentArray) {
    ...
    if (hasValidSignature(item as ThinkingPart)) {
      filtered.push(sanitizeThinkingPart(item as ThinkingPart));
      continue;
    }
 ...
 *
 * @param contents - Array of content objects in Gemini format
 * @returns Filtered contents with unsigned thinking blocks removed
 */
export function filterUnsignedThinkingBlocks(contents: { role: string; parts: GooglePart[] }[]): { role: string; parts: GooglePart[] }[] {
  return contents.map((content) => {
    if (!content || typeof content !== "object") return content;

    if (Array.isArray(content.parts)) {
      return { ...content, parts: filterContentArray(content.parts) as GooglePart[] };
    }
 ...
 * @returns Content array with trailing unsigned thinking blocks removed
 */
export function removeTrailingThinkingBlocks(content: AnthropicContentBlock[]): AnthropicContentBlock[] {
  ...
  for (let i = content.length - 1; i >= 0; i--) {
    ...
    const isThinking = isThinkingPart(block as ThinkingPart);

    if (isThinking) {
      // Check if it has a valid signature
      if (!hasValidSignature(block as ThinkingPart)) {
        endIndex = i;
      } else {
 ...
 *
 * @param content - Array of content blocks
 * @returns Filtered content with only valid signed thinking blocks
 */
export function restoreThinkingSignatures(content: AnthropicContentBlock[]): AnthropicContentBlock[] {
  if (!Array.isArray(content)) return content;
  ...
  for (const block of content) {
    ...
    const thinkingBlock = block as ThinkingPart;
    // Keep blocks with valid signatures (>= MIN_SIGNATURE_LENGTH chars), sanitized
    if (thinkingBlock.signature && thinkingBlock.signature.length >= MIN_SIGNATURE_LENGTH) {
      filtered.push(sanitizeAnthropicThinkingBlock(thinkingBlock) as AnthropicContentBlock);
    }
 ...
 */
export function reorderAssistantContent(content: AnthropicContentBlock[]): AnthropicContentBlock[] {
  if (!Array.isArray(content)) return content;
  ...
  if (content.length === 1) {
    ...
    if (block && (block.type === "thinking" || block.type === "redacted_thinking")) {
      return [sanitizeAnthropicThinkingBlock(block as ThinkingPart) as AnthropicContentBlock];
    }
    return content;
  }
  ...
  for (const block of content) {
    ...
    if (block.type === "thinking" || block.type === "redacted_thinking") {
      ...
      thinkingBlocks.push(sanitizeAnthropicThinkingBlock(block as ThinkingPart) as AnthropicContentBlock);
 ...
 * @returns True if message has valid signed thinking blocks
 */
function messageHasValidThinking(message: AnalyzableMessage): boolean {
  const content = message.content ?? message.parts ?? [];
  if (!Array.isArray(content)) return false;
  return content.some((block) => {
    const thinkingBlock = block as ThinkingPart;
    if (!isThinkingPart(thinkingBlock)) return false;
    // Check for valid signature (Anthropic style)
    if (thinkingBlock.signature && thinkingBlock.signature.length >= MIN_SIGNATURE_LENGTH) return true;
    // Check for thoughtSignature (Gemini style on functionCall)
    if (thinkingBlock.thoughtSignature && thinkingBlock.thoughtSignature.length >= MIN_SIGNATURE_LENGTH) return true;
    return false;
  });
}
 ...
 * @param message - Message to check
 * @returns True if message has tool_use blocks
 */
function messageHasToolUse(message: AnalyzableMessage): boolean {
  const content = message.content ?? message.parts ?? [];
  if (!Array.isArray(content)) return false;
  return content.some((block) => {
    const b = block;
    return (b as AnthropicContentBlock).type === "tool_use" || "functionCall" in b;
  });
}
 ...
 * @param message - Message to check
 * @returns True if message has tool_result blocks
 */
function messageHasToolResult(message: AnalyzableMessage): boolean {
  const content = message.content ?? message.parts ?? [];
  if (!Array.isArray(content)) return false;
  return content.some((block) => {
    const b = block;
    return (b as AnthropicContentBlock).type === "tool_result" || "functionResponse" in b;
  });
}
 ...
 * @param message - Message to check
 * @returns True if message is plain user text
 */
function isPlainUserMessage(message: AnalyzableMessage): boolean {
 ...
 *
 * @param messages - Array of messages
 * @returns State object with inToolLoop, interruptedTool, turnHasThinking, etc.
 */
export function analyzeConversationState(messages: AnalyzableMessage[]): ConversationState {
 ...
 *
 * @param messages - Array of messages
 * @returns True if thinking recovery is needed
 */
export function needsThinkingRecovery(messages: AnalyzableMessage[]): boolean {
 ...
 *
 * @param messages - Array of messages
 * @param targetFamily - Target model family ('claude' or 'gemini')
 * @returns Messages with invalid thinking blocks removed
 */
function stripInvalidThinkingBlocks(messages: AnalyzableMessage[], targetFamily: ModelFamily | null = null): AnalyzableMessage[] {
  let strippedCount = 0;
  ...
  const result = messages.map((msg) => {
    ...
    const filtered = content.filter((block) => {
      ...
      if (!isThinkingPart(thinkingBlock)) return true;

      // Check generic validity (has signature of sufficient length)
      if (!hasValidSignature(thinkingBlock)) {
        strippedCount++;
        return false;
      }
      ...
      if (targetFamily === "gemini") {
        const signature = thinkingBlock.thought === true ? thinkingBlock.thoughtSignature : thinkingBlock.signature;
    ...
    if (msg.content) {
      ...
      return { ...msg, parts: filtered.length > 0 ? filtered : [{ text: "." }] } as AnalyzableMessage;
    }
    return msg;
  });
 ...
 * @param messages - Array of messages
 * @param targetFamily - Target model family ('claude' or 'gemini')
 * @returns Modified messages with synthetic messages injected
 */
export function closeToolLoopForThinking(messages: AnalyzableMessage[], targetFamily: ModelFamily | null = null): AnalyzableMessage[] {
  const state = analyzeConversationState(messages);
