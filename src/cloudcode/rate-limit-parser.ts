...
/**
 * Interface for Response-like objects
 */
 ...
 *
 * @param responseOrError - HTTP Response object or Error
 * @param errorText - Optional error body text
 * @returns Reset time in milliseconds or null if not found
 */
export function parseResetTime(responseOrError: ResponseLike | Error | null | undefined, errorText = ""): number | null {
  let resetMs: number | null = null;

  // If it's a Response object, check headers first
  if (responseOrError && typeof (responseOrError as ResponseLike).headers?.get === "function") {
    const headers = (responseOrError as ResponseLike).headers!;
  ...
  if (resetMs === null) {
    ...
    const quotaDelayMatch = /quotaResetDelay[:\s"]+(\d+(?:\.\d+)?)(ms|s)/i.exec(msg);
    if (quotaDelayMatch?.[1] && quotaDelayMatch[2]) {
    ...
    if (resetMs === null) {
      const quotaTimestampMatch = /quotaResetTimeStamp[:\s"]+(\d{4}-\d{2}-\d{2}T[\d:.]+Z?)/i.exec(msg);
      if (quotaTimestampMatch?.[1]) {
    ...
    if (resetMs === null) {
      const durationMatch = /(\d+)h(\d+)m(\d+)s|(\d+)m(\d+)s|(\d+)s/i.exec(msg);
      if (durationMatch) {
    ...
    if (resetMs === null) {
      const isoMatch = /reset[:\s"]+(\d{4}-\d{2}-\d{2}T[\d:.]+Z?)/i.exec(msg);
      if (isoMatch?.[1]) {
